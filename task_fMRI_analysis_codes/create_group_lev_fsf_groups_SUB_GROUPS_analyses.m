function create_group_lev_fsf_groups_SUB_GROUPS_analyses(model_name, analysis_name, lev1CopeNum, group, variance_equality, region)
% Created based on the functions function of the group-level
% analysis of the main analysis.
% Example: create_group_lev_fsf_groups_SUB_GROUPS_analyses('002', 'last2_vs_first2_runs', 8, 'short', 'equal_var_assumed', 'whole_brain')
...
% get/define launch file full path:
launchFilePath = fullfile(launchdir, ['group_model' model_name '_' group '_SubGroups_' analysis_name '_' variance_equality '_based-on-' onsetsOrWholeBlocks '_' region '_launch.txt']);
...
fprintf(['\n** Creating group level fsfs for model ' model_name '_' group ' SubGroups ' analysis_name ' based on ' onsetsOrWholeBlocks ' - ' variance_equality ' - ' region ' **\n'])
% read the fsf template file into a variable
fsfTemplate = fileread(fsfTemplateFile);

% Change EV titles:
if any(strcmp(group, {'short','long'}))
...
elseif any(strcmp(group, {'habitual','goal_directed'}))
    fsfTemplate = strrep(fsfTemplate, 'set fmri(evtitle1) ""','set fmri(evtitle1) "1-Day"');
    fsfTemplate = strrep(fsfTemplate, 'set fmri(evtitle2) "habit index"','set fmri(evtitle2) "3-Days"');
    fsfTemplate = strrep(fsfTemplate, 'Habitual > Goal-Directed','1-Day > 3-Days');
    fsfTemplate = strrep(fsfTemplate, 'Goal-Directed > Habitual','3-Days > 1-Day');
    fsfTemplate = strrep(fsfTemplate, 'Habitual subgroup mean','1-Day subgroup mean');
    fsfTemplate = strrep(fsfTemplate, 'Goal_Directed subgroup mean','3-Days subgroup mean');
end
...
if strcmp(group, 'long')
    behav_data_of_group = behav_data(behav_data.ID > 200 & behav_data.ID < 300,:);
elseif strcmp(group, 'short')
    behav_data_of_group = behav_data(behav_data.ID > 100 & behav_data.ID < 200,:);
elseif strcmp(group, 'habitual')
    behav_data_of_group = behav_data(strcmp('Habitual', behav_data.Cluster),:);
elseif strcmp(group, 'goal_directed')
    behav_data_of_group = behav_data(strcmp('Goal-directed', behav_data.Cluster),:);
end

% Remove folders of subjects that do not appear in the behavor file:
lev2feat_full_paths = lev2feat_full_paths(ismember(cellfun(@str2double, subjects)', behav_data_of_group.ID)); % the order of this two lines is important
subjects = subjects(ismember(cellfun(@str2double, subjects)', behav_data_of_group.ID));

% mean_centering:
behav_data_of_group.habit_index_mean_centered = behav_data_of_group.habit_score - mean(behav_data_of_group.habit_score);
% check that fmri data and behavioral data have the same subjects:
if ~isequal(behav_data_of_group.ID, cellfun(@str2double, subjects)')
    % there is a fix for the case of 276 who's scan during extinction was
    % in AC-PC 0 and thus excluded from this analysis.
    if strcmp(analysis_name, 'extinction_vs_last_run') && behav_data_of_group.ID(~ismember(behav_data_of_group.ID,cellfun(@str2double, subjects)')) == 276
        behav_data_of_group = behav_data_of_group(behav_data_of_group.ID~=276,:);
    else
...
for ind = 1:length(lev2feat_full_paths)
    ...
    if any(strcmp(group, {'short','long'}))
        ev1_value_assignment = [ev1_value_assignment '# Higher-level EV value for EV 1 and input ' num2str(ind) newline ...
            'set fmri(evg' num2str(ind) '.1) ' num2str(strcmp(behav_data_of_group.Cluster{ind},'Habitual')) newline newline];
        ev2_value_assignment = [ev2_value_assignment '# Higher-level EV value for EV 2 and input ' num2str(ind) newline ...
            'set fmri(evg' num2str(ind) '.2) ' num2str(strcmp(behav_data_of_group.Cluster{ind},'Goal-directed')) newline newline];
    elseif any(strcmp(group, {'habitual','goal_directed'}))
        ev1_value_assignment = [ev1_value_assignment '# Higher-level EV value for EV 1 and input ' num2str(ind) newline ...
            'set fmri(evg' num2str(ind) '.1) ' num2str(strcmp(behav_data_of_group.group{ind},'Short training')) newline newline];
        ev2_value_assignment = [ev2_value_assignment '# Higher-level EV value for EV 2 and input ' num2str(ind) newline ...
            'set fmri(evg' num2str(ind) '.2) ' num2str(strcmp(behav_data_of_group.group{ind},'Extensive training')) newline newline];
    end
    ...
    elseif strcmp(variance_equality, 'unequal_var_assumed')
        if any(strcmp(group, {'short','long'}))
            group_membership_value_assignment = [group_membership_value_assignment '# Group membership for input ' num2str(ind) newline ...
                'set fmri(groupmem.' num2str(ind) ') ' num2str(group_num_options(strcmp(behav_data_of_group.Cluster{ind},{'Habitual','Goal-directed'}))) newline newline];
        elseif any(strcmp(group, {'habitual','goal_directed'}))
            group_membership_value_assignment = [group_membership_value_assignment '# Group membership for input ' num2str(ind) newline ...
                'set fmri(groupmem.' num2str(ind) ') ' num2str(group_num_options(strcmp(behav_data_of_group.group{ind},{'Short training','Extensive training'}))) newline newline];
        end
    end
end
...
targetFilePath=fullfile(fsfdir, ['design_group_model-' model_name '_' group '_SubGroups_' analysis_name '_' variance_equality '_based-on-' onsetsOrWholeBlocks '_' region '.fsf']);
...
if ~exist(targetFilePath, 'file') || rewrite_fsfs || rewrite_launchfiles
    ...
    if ~exist(targetFilePath, 'file') || rewrite_fsfs
        ...
        replacements = containers.Map({'DATAANALYSISDIR', 'FSLLOCALDIR', 'MODELNAME', 'ANALYSIS_TYPE_ONSETS_OR_BLOCKS','ANALYSIS_NAME', 'VARIANCE_EQUALITY', 'REGION', 'MASK_FILE', 'NVOLS', 'LEV_2_COPE_INFO', 'FEAT_FILES_FROM_SECOND_LEVEL', 'EV1_VALUE_ASSINGMENTS', 'EV2_VALUE_ASSINGMENTS', 'GROUP_MEMBERSHIP_VALUE_ASSIGNMENT'}, ...
            { dataAnalysisDir ,  fslLocalDir,   model_name,  analysis_type_onsets_or_blocks ,[group '_SubGroups_' analysis_name] ,variance_equality , region ,  mask_file ,  nvols ,  lev_2_cope_info ,  feat_files_from_lev2         ,  ev1_value_assignment  ,  ev2_value_assignment  ,group_membership_value_assignment });
...
fprintf(['** Creating group level fsfs and launch files for "' group ' SubGroups ' analysis_name '" in model ' model_name ' based on ' onsetsOrWholeBlocks ' - ' region ' COMPLETED **\n'])
