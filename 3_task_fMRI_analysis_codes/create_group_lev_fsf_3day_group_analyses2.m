function create_group_lev_fsf_3day_group_analyses2(model_name, analysis_name, lev1CopeNum, region)
...
launchFilePath = fullfile(launchdir, ['noFieldMapParticipants_group_model' model_name '_' analysis_name '_based-on-' onsetsOrWholeBlocks '_' region '_launch.txt']);
...
for folderInd = 1:length({lev2feat_folders.name})
    ...
    if ~any(strcmp(num2str(subNum), subjects))
        if subNum > 200 && subNum < 300 && ~ismember(subNum, subjectsToExclude)
...
behav_data(:,2:end-1) = [];
behav_data_3day_group = behav_data(behav_data.ID > 200 & behav_data.ID < 300 & ~ismember(behav_data.ID, subjectsToExclude),:);
% mean_centering:
...
targetFilePath=fullfile(fsfdir, ['noFieldmapParticipants_design_group_model-' model_name '_' analysis_name '_based-on-' onsetsOrWholeBlocks '_' region '.fsf']);
