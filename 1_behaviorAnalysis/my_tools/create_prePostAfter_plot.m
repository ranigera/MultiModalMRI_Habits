function graph = create_prePostAfter_plot (pattern, data1, data2, data3, xtags)
...
mean_afterExperiment = structfun(@nanmean,  data3);
mean_afterExperiment = mean_afterExperiment (1:2);
std_afterExperiment = structfun(@(x) nanstd(x)/sqrt(length(x)), data3);
std_afterExperiment = std_afterExperiment (1:2);
...
b = bar([mean_pre(1), mean_pre(2); mean_post(1), mean_post(2); mean_afterExperiment(1), mean_afterExperiment(2)],0.95);
b(1).FaceColor = color.means.val;
b(2).FaceColor = color.means.deval;


% plot errorbars
model_series = [mean_pre(1), mean_pre(2); mean_post(1), mean_post(2); mean_afterExperiment(1), mean_afterExperiment(2)];
model_error =  [std_pre(1), std_pre(2); std_post(1), std_post(2); std_afterExperiment(1), std_afterExperiment(2)];
...
ypos = min(ylim)-0.005;
labelx = xtags;
text([0.85  1.85  2.70],repmat(ypos,length(labelx),1), ...
