...
if isfield(var, 'runs') % an indicator that it is inside the MRI
    p_reward_availability = 0.1; % i.e. the regular task in the scanner. RI-10
    ...
    p_reward_availability = 0.2; % make schedule RI-5 for practice round
    switch var.condition(ii)
        case 1 % sweet
            position_response_cue = CenterRectOnPointd(CUEbaseRect,var.sweet_square_pos_demo, var.yUpper);
            fractal               = var.sweet_fractal_demo;
            response              = var.sweet_action_demo;
            reward                = var.sweetImage;
            
        case 2 % salty
            position_response_cue = CenterRectOnPointd(CUEbaseRect,var.salty_square_pos_demo, var.yUpper);
            fractal               = var.salty_fractal_demo;
            response              = var.salty_action_demo;
            reward                = var.saltyImage;
            
        case 0 % baseline (just show rest block)
            fractal               = var.rest_fractal_demo;
            timer = GetSecs()-var.time_MRI;
            while timer < var.ref_end
                timer = GetSecs()-var.time_MRI;
                % baseline screen
...
slideOnT = GetSecs();
timer = GetSecs()-var.time_MRI;
KbQueueCreate([], var.availableKeysInTask);
KbQueueStart();
    ...
    if keyPressed  
        ...
        if (keyPressed == 1) && ismember(respKey, response) % Register the accuracy of the response
            ...
            if availability == 1
                ...
                availability = 0; % reset availability to 0 % MOVED HERE BY RANI, So it becomes zero only if a reward was delivered!
            else
